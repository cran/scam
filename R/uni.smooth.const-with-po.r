...
{ 
  ...
  X <- X1[,2:q] ## %*%Sig # model submatrix for the positive term
...
####################################################
### Adding a SCOP-spline construction with positivity constraint 
######################################################


smooth.construct.po.smooth.spec<- function(object, data, knots)
## construction of the positive smooth...
{ 
  m <- object$p.order[1]
  if (is.na(m)) m <- 2 ## default for cubic spline
  if (m<1) stop("silly m supplied")
  ...
  #  get model matrix-------------
  X1 <- splineDesign(xk,x,ord=m+2)
  # use matrix Sigma which is just an identitymatrix for positive constraint 
  Sig <- diag(1,q)   
...
{ m <- object$m+1; # spline order, m+1=3 default for cubic spline
  q <- object$df +1
  Sig <- diag(q)  # Define Matrix Sigma
